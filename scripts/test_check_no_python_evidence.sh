...
# The real clean checkout must continue to pass when rg is available.
if ! (cd "$repo_root" && /bin/bash scripts/check_no_python_evidence.sh) \
    >"$tmpdir/clean.out" 2>"$tmpdir/clean.err"; then
...
printf '%s\n' 'check_no_python_evidence regressions ok: clean tree passes; missing rg fails closed; stale references are rejected'
