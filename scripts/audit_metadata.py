...
EXPECTED_WORDING = [
    ...
    "Aggregate top-up budget conservation is proved at MODEL, SOURCE, and bounded Verity transaction planes, conditional on independently supplied runtime provenance. The TX plane is an adversary-quantified CallProgram model derived from the pinned source transition, not a Contract.run execution observation; it constrains the sum of dynamically observed call values and does not establish callee address, call kind, calldata, per-validator call granularity, or failed-call provenance. EVM verifier-binding remains BLOCKED.",
