...
EXPECTED_WORDING = [
    ...
    "Concrete Verity model evidence represents the pinned WithdrawalVault consolidation entrypoint: immutable-target fee STATICCALL, exact dynamic bytes[] validation, checked exact fee, 96-byte request calls, and post-success events. A.1 proves rollback only when every observed call rolls back; rollback of earlier successful calls after a later failure remains explicitly open.",
...
EXPECTED_REPRODUCTION = [
    ...
    {"command": "lake build LidoSRv3.Audit.Verity.ConsolidationFee", "expected": "successful typed FunctionSpec compilation, bytes[]/96-byte memory proofs, exact fee call program, and the bounded A.1 observed-call rollback theorem"},
...
EXPECTED_NEXT_GATES = [
    ...
    "Add a Verity transaction-frame denotation that rolls back earlier successful mutable calls and logs after any later failure; then refine generated Yul/EVM and runtime provenance.",
...
EXPECTED_STATUSES = [
    ...
    {"model": "LEAN_CHECKED", "algorithm": "NOT_APPLICABLE", "source": "OPEN", "tx": "OPEN", "yul": "OPEN", "evm": "BLOCKED", "crypto": "NOT_APPLICABLE"},
...
EXPECTED_THEOREMS = [
    ...
    "LidoSRv3.Audit.Verity.ConsolidationFee.batch_all_observed_calls_rollback",
