...
EXPECTED_WORDING = [
    ...
    "Source-shaped deposit prefix scaffold (OPEN): the Verity FunctionSpec places the callerHasDepositRole interface guard first, then compiles locator-derived DSM authentication, module membership/config extraction, withdrawal-credentials conversion, immutable LIDO.getDepositableEther, and 32-byte successful-returndata checks. AccessControl storage derivation, allocation, and the multi-contract suffix remain OPEN; this is not a full source, transaction, conservation, or rollback proof.",
...
EXPECTED_REPRODUCTION = [
    ...
    {"command": "lake build LidoSRv3.Audit.Verity.DepositRollback LidoSRv3.Audit.Verity.Tests.DepositRollback", "expected": "successful OPEN prefix-scaffold compilation, leading DEPOSIT_ROLE interface guard, 32-byte successful-returndata guards, actual FunctionSpec malformed-ABI rejection, independently declared expected-footprint comparison, and call-site-sensitive immutable-target guards; AccessControl storage derivation, malformed-ABI snapshot rollback, and the full source/transaction path remain OPEN"},
