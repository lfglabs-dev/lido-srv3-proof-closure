...
private def validator (effective pending : Nat) : Validator :=
  ⟨#[], 1, 2, true, false, false, effective, pending⟩

private def cfg : TopupConfig := ⟨64, 4, 100, 8, 80, 3600⟩

/- Omitted-pending and wrong-threshold mutants disagree with pinned branches. -/
example : evaluated_topup_limit (validator 50 10) cfg = 4 := by decide
example : (64 - (validator 50 10).effectiveBalanceGwei) ≠
    evaluated_topup_limit (validator 50 10) cfg := by decide
example : evaluated_topup_limit (validator 61 0) cfg = 0 := by decide
example : (64 - 61) ≠ evaluated_topup_limit (validator 61 0) cfg := by decide

/- Exiting/slashed, target-reached, below-minimum, and accepted-gap branches. -/
example : evaluated_topup_limit { validator 1 0 with exiting := true } cfg = 0 := by decide
example : evaluated_topup_limit { validator 1 0 with slashed := true } cfg = 0 := by decide
example : evaluated_topup_limit (validator 64 0) cfg = 0 := by decide
example : evaluated_topup_limit (validator 60 0) cfg = 4 := by decide
...
example : (GatewayPackedContract.recordBudget 11 10).run Verity.defaultState =
    .revert "AGGREGATE_OVER_BUDGET" Verity.defaultState := by decide
