...
private def vector (name : String) (s : Storage) (oracle : StaticOracle)
    (inp : CapacityInput := input) : String :=
  let (result, trace) := produce layout s oracle inp []
...
def solidityVectors : String := array [
  vector "mixed" (storage 2 (packed 21 5000 0 1) (packed 22 5000 1 2)) honest,
  vector "underflow-before-stake" (seed 1 10000 0 2) underflow,
  vector "late-rejection" (seed 2) lateReject,
  vector "short-summary" (seed) (responseOracle (List.replicate 95 (byte 0)) [] []),
  vector "short-stake" (seed 1 10000 0 2)
    (responseOracle (summary 0 1 1) [] (List.replicate 31 (byte 0))),
  vector "trailing-summary" (seed)
    (responseOracle (summary 0 1 1 ++ [byte 0xff]) [] []),
  vector "zero-ceil-divisor" (seed 1 10000 0 2)
    (responseOracle (summary 0 1 1) [] (encodeWord (word 0)))
    { input with config := { maxEBType1 := word 0, maxEBType2 := word 2048 } },
  vector "invalid-enum-before-call" (seed 1 10000 3) standard,
  vector "below-allocation" (seed 1 0) standard,
  vector "topup-product-before-division" (seed 1 10000 0 2)
    (responseOracle (summary 0 2 0) [] (encodeWord (word 0)))
    { input with isTopUp := true, config := { maxEBType1 := word 1, maxEBType2 := word (2^256-1) } },
  vector "empty-zero-divisor" (seed 0) standard
    { input with config := { maxEBType1 := word 0, maxEBType2 := word 0 } },
  vector "total-overflow-before-next-row" (seed 2) standard
    { input with depositsToAllocate := word (2^256-1) }
]

end LidoSRv3.Tests.TrioAlloc1

def main : IO Unit := IO.println LidoSRv3.Tests.TrioAlloc1.solidityVectors
