...
namespace LidoSRv3.Tests.TrioReserve1.ReportDifferential
...
def execute (j : Json) : Except String Json := do
  ...
  if [self,sender,locator,vault,queue].any (· ≥ 2^160) then throw "address overflow"
  ...
  let core := {Verity.defaultState with codeSize := fun a =>
    if a = locator ∨ a = vault ∨ a = queue then word 1 else word 0}
  ...
  let before : World := ⟨core, fun a => if a.val = self then balance else if a.val = queue then queueBalance else 0, []⟩
  ...
  let returned := match r.outcome with
    | .error (.bubbled data) => data.map UInt8.toNat
    | _ => []
  pure <| Json.mkObj [
    ("name", ← j.getObjVal? "name"), ("fault", .str fault), ("bubbled", toJson returned),
    ...
    ("calls", toJson (r.attempts.map fun a => Json.mkObj [
      ("target", .str (toString a.request.target.val)), ("value", .str (toString a.request.value.val)),
...
end LidoSRv3.Tests.TrioReserve1.ReportDifferential
...
def main (args : List String) : IO UInt32 := do
  ...
  | [input, output] =>
    ...
    match Lean.Json.parse raw >>= fun j => j.getArr? >>= fun rows =>
        rows.mapM LidoSRv3.Tests.TrioReserve1.ReportDifferential.execute with
    | .error e => IO.eprintln e; pure 1
    | .ok rows => IO.FS.writeFile output (Lean.Json.arr rows |>.pretty); pure 0
  | _ => IO.eprintln "usage: ReportDifferential.lean INPUT OUTPUT"; pure 2
