...
The ordinary-account premise explicitly excludes Cancun precompiles. -/
theorem actual_module_no_code_failure (hash : TopupRouterCredentials.Keccak) (m x : External)
    ...
    (hc : audit.trio.consolidation.emptyCodeAccount before (TopupModuleCall.moduleAddress hash ctx.sender i.moduleId before)) :
...
theorem actual_root_batch_no_code_failure (hash : TopupRouterCredentials.Keccak) (m x : External)
    ...
    (hc : audit.trio.consolidation.emptyCodeAccount e.before (TopupModuleCall.moduleAddress hash ctx.sender moduleId e.before)) :
