...
def parseInputs (json : String) : Option Inputs := do
  ...
  -- `maxEBType1` is the `MAX_EFFECTIVE_BALANCE_WC_TYPE_01` immutable
  -- (StakingRouter.sol:972 pull denominator). Harness JSON may omit it;
  -- on a conserving deployment the pin sets it equal to `DEPOSIT_SIZE`.
  let maxEBType1 :=
    match wordField json "maxEBType1" with
    | some w => w
    | none => depositSize
  ...
  pure {
    ...
    depositSize, maxEBType1, lido, module, beacon, batches
  }
