...
def encode (canDeposit authorizedRouter : Bool) (amount : Nat)
  ...
  -- 2026-09-13 chantier 2 Piste A: `WithdrawInputs` no longer carries raw
  -- `canDeposit` / `authorizedRouter` Bools.  Construct the pinned
  -- LidoStakingState and ACLState wrappers so their `def` accessors
  -- recover the requested Bool values on the CLI plane.
  let inputs : WithdrawInputs := {
    lidoState := {
      isStakingPaused := !canDeposit
      isBunkerActive := false
    }
    acl := {
      stakingRouterRole := authorizedRouter
      topUpGatewayApp := false
    }
  }
