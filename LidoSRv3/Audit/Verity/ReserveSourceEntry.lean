...
def encode (canDeposit authorizedRouter : Bool) (amount : Nat)
  ...
  -- Post-2026-09-13 chantier 2 shape: `WithdrawInputs` carries pinned
  -- `LidoStakingState` and `ACLState` records, with `canDeposit` and
  -- `authorizedRouter` as `@[reducible, simp] def` accessors.
  let inputs : WithdrawInputs :=
    { lidoState := { isStakingPaused := !canDeposit, isBunkerActive := false }
      acl := { stakingRouterRole := authorizedRouter, topUpGatewayApp := false } }
