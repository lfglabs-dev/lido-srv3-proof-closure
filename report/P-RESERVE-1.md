...
## Reserve-target setter access control (2026-09-18)

[PReserve1TargetAdmission](../LidoSRv3/Audit/Guarantees/PReserve1TargetAdmission.lean) composes the public setter `Lido.setDepositsReserveTarget` (`Lido.sol:656-660`): `_auth(BUFFER_RESERVE_MANAGER_ROLE)` is `Aragon.canPerform`, an actual CALL into the kernel address stored in Lido's `kernelSlot`, decoded to a word, followed by the internal writer only when it is nonzero. `executeGuarded` runs this for `Action.target` and the registered writer for the other actions; `actual_reserve_guarded_physical_history` threads the returned worlds as `actual_reserve_physical_history` does, with `target_success` (accepted kernel reply and writer specification on the kernel-returned world) and `target_denied` (`APP_AUTH_FAILED`, entry world, kernel/ACL trace). `physical_target_allowed` / `physical_target_denied` instantiate the Lido→Kernel→ACL interpreter, so admission and denial follow from the stored permission graph. The setter's ABI frame is one `uint256` argument under the shared source-shaped entry assumption. Axioms: `propext`, `Classical.choice`, `Quot.sound`.
