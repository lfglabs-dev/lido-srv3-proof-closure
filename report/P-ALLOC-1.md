...
The [allocated-key arithmetic prefix](../LidoSRv3/Audit/Source/NodeOperatorsRegistry/AllocatedKeys.lean) retains raw uint256 wrapping, the equal-value continuation, and Solidity 0.4.24 INVALID on assertion failure. `allocatedKeyPrefix_no_overflow` derives the nonwrapping sum from successful prefix execution and incoming local exited ≤ deposited; `allocatedKeyPrefix_load_delta` derives the exact positive number of keys to load. A regression preserves the wrap-to-equal continuation possible on invalid input accounting, which a checked-add replacement would erase. This is only the iteration prefix: array accesses, integration of SigningKeys memory writes, packed deposited writes, aggregate updates, and complete loader invariant preservation remain open.

The [SigningKeys library block](../LidoSRv3/Audit/Source/SigningKeys.lean) uses existing EvmYul byte-memory stores for the overlapping public-key writes and three signature words. It retains wrapping indices and storage addresses, including aliasing between buffers. Its loop relation uses the source word guard; `loadKeysSigs_execution` connects finite recursion to that relation without a fuel premise. The slot producer threads its updated machine state so ABI-encoding memory effects are not silently erased. Instantiating that encoder/hash, account-storage binding, allocation, compiler control flow and gas/OOG remain outside this primitive block. The shared [byte-memory lemmas](../LidoSRv3/Audit/Source/ByteMemory.lean) derive exact reads, writes and unchanged memory size when the slices fit, without a fixed small-address cutoff. Establishing those slice-fit conditions for the complete NOR loader remains required. The finite memory regressions are not a universal buffer-layout or full NOR-loader theorem. The [Solidity control harness](../solidity/signing-keys/test/SigningKeysMemory.t.sol) imports the pinned library through a 0.4.24 harness, including its actual packed key-slot hash, and checks overlapping writes, buffer aliasing and start-index wrapping across two iterations. Run it with `lake test -- signing-keys-source`; it is also part of the repository gate. These finite controls do not establish compiler correspondence or complete loader closure; their validation status must be read from the candidate receipts.
