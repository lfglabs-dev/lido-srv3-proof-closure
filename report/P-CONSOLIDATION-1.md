...
`source_consolidation_preserves_eligibility_value_atomicity_from_gateway` (chantier 2 Thomas 2026-09-13 item a, PR #699) is that characterization, under `PredeployStaticcallResult`-shaped pinned-source premises (naming the EIP-7251 STATICCALL return at `WithdrawalVaultEIP7685.sol:79-93`) plus the gateway-boundary bridge `hMsgValue`, `hCountPos`, and `hFeeNonzero`. Under those, `caller = gateway → msg.value ≠ 0` is DERIVED via `gatewayTotalFee_ne_zero_of_fee_ne_zero`, and `A-CONSOLIDATION-GATEWAY-NONZERO` is RETIRED from this guarantee's assumption list. `verity_tx_simulates_consolidation` matches `sourceView` when four memory arrays decode. We do not cover beacon eligibility, grouping, quota, or the Bus. Do not compose with P-CONSOLIDATION-ETH-1.

## Proof limitations and recommendations

The registered parent is an unbounded $\forall$ hypothesized on `PredeployStaticcallResult` + bridge premises (post-PR #699 shape). `hGatewayAdmittedNonzero` is derived internally. The `ConsolidationGateway.sol:189` guards the gateway's own `msg.value`, not the vault's forwarded `totalFee`; the `fee = 0` on-chain case is documented by `gatewayTotalFee_zero_at_fee_zero` (source plane). `preservesEthBalance` is a `String` gap: journaled CALLs are pre-marked successful and move no wei. Lean and YAML now state the pinned obligation precisely: the vault forwards exactly `msg.value`. Verity rollback is the `Contract.run` combinator, true of every program. Fabricated `sourceMapSlot`/`targetMapSlot` slots are physically retired (PR #683); the registered parent consumes `observeFromJournal` on `addRequestsSlotFree`.

CHECKED does not mean eligibility in the beacon sense, that the vault paid $n \times \mathrm{fee}$, or gateway grouping.

Ranked next work: full executable `Contract.run` frame composition via `abi.encodeCall` from the gateway's Verity contract to the vault's `Contract.run`; live-STATICCALL executable model on the pinned EIP-7251 predeploy to discharge the residual `hFeeNonzero` premise (multi-session Verity model work). Outer-gateway nonzero-value premise retirement was landed via PR #699. Fabricated slot machinery was physically retired via PR #683.

Theorems: `PConsolidation1.source_consolidation_preserves_eligibility_value_atomicity_from_gateway` (registered abstract parent, post-PR #699), `PConsolidation1.source_consolidation_preserves_eligibility_value_atomicity` (kept as unregistered evidence for downstream compat), `PConsolidation1.verity_tx_simulates_consolidation`, `PConsolidation1.verity_tx_simulates_consolidation_from_gateway` (ABI-bridge parent, PR #644), `PConsolidation1.verity_tx_gateway_bridge_derives_nonzero_msg_value` (PR #647), `PConsolidation1.fee_blind_commit_kill_line_refutes_parent`, `PConsolidation1.gateway_admitted_nonzero_kill_line`.
Assumptions: `A-SOURCE-SHAPED`, `A-VERITY-SCAFFOLD`, `A-SOLC-TRUSTED`, `A-RUNTIME-PROVENANCE` (A-CONSOLIDATION-GATEWAY-NONZERO retired via PR #699).
