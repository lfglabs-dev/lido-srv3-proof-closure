...
## Lazy calldata accessors (2026-09-19)

[PConsolidation1VaultLazy](../LidoSRv3/Audit/Guarantees/PConsolidation1VaultLazy.lean) models the vault dispatcher with solc's lazy `bytes[] calldata` accessors (`lazyEntry`, `lazyLoop`) at their source positions (`WithdrawalVaultEIP7685.sol:56-73`) and relates it to the eager dispatcher: `lazy_eq_eager_wellFormed` (same reply on every argument block the eager decoder accepts), `lazy_rejects_malformed` (rejection on every one it rejects), `eager_caller_restored` and `lazy_caller_restored` (the caller's world is untouched under both). The difference on malformed input is confined to the revert data and the attempt trace. Axioms: `propext`, `Classical.choice`, `Quot.sound`.
