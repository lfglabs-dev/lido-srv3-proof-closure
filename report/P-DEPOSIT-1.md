...
Theorems: `PDeposit1.source_deposit_conserves_and_rolls_back` (registered abstract parent), `PDeposit1.verity_tx_composes_deposit_conservation_and_rollback` (Verity composition), `PDeposit1.linked_deployment_push_is_word_bounded`, `PDeposit1.linked_conserving_deployment_pull_is_word_bounded` and `PDeposit1.abstract_parent_covers_deployments_the_verity_plane_omits` with `PDeposit1.oversized_run_commits` and `PDeposit1.oversized_input_is_outside_the_source_domain` (checked statement of the finite executable scope, of the abstract-model inputs it omits, and of those inputs' deliberate distance from the uint256 source domain), `PDeposit1.linked_hypotheses_do_not_bound_the_source_pull` (pull kill-line with a uint256-encodable immutable) with `PDeposit1.skewed_pull_witness_turned_away_before_line_972` (executable regression: the skewed witness turns away at the line-959 `ZeroDeposits` guard before the line-972 multiplication) and `PDeposit1.line_972_product_le_module_allocation` with `PDeposit1.encodable_allocation_bounds_line_972_product` (the line-969 guard bounds the line-972 product by the module allocation, so an encodable allocation keeps it inside one word), `DepositVectors.dropped_conservation_assert_breaks_pulled_eq_pushed` (kill-line).
...
- The executable transaction is an exactly-two-batch unrolling of the source deposit loop, not a proved bound on the loop. That is now explicit and checked (conjunct (d), `linked_deployment_push_is_word_bounded`, `abstract_parent_covers_deployments_the_verity_plane_omits` with `oversized_run_commits`) but does not close it. Closing it requires an `execute` over a list of batches with an inductive journal and observables correspondence, together with a no-wrap argument that survives the fold; the two-leg stage lemmas in `DepositParentTx` do not generalize for free.
