...
## Available ETH and allocation calls (2026-09-18)

[PDeposit1AllocationViews](../LidoSRv3/Audit/Guarantees/PDeposit1AllocationViews.lean) executes the two free inputs of the registered parent's allocation limb: `lidoDepositableEther` is Lido's `getDepositableEther()` view (`Lido.sol:823-825`, the existing `getBufferedEtherAllocation` body with its locator and queue STATICCALLs), `readDepositableEther` the router's STATICCALL to it, and `execute` is `DepositAllocation.execute` with that view at its source position. `deposit_with_lido_view`: a success consumed `available = depositsReserve + unreserved` of the allocation Lido computed on the entry world and the registered effects hold at it; every failure restores the entry world. `allocation_view_is_vm_execution`: the allocation view the chain consumes is the Verity-VM allocation execution of P-ALLOC-1's `account_allocation_result` when the VM world is the router's frame of the entry world. Cursor origins and deployed identities remain inputs. Axioms: `propext`, `Classical.choice`, `Quot.sound`.
