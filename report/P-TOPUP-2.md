...
**Wave 3 (2026-09-13, chantier 2 mandate).** Two model corrections landed on the registered Verity parent `verity_tx_simulates_topup2_spec` per the 2026-09-13 Piste B mandate; several D-* divergences from Grok #417 remain open. Neither correction has retired the leftover-budget walk (`sourceConsume`); both are threaded through the same walk.

(a) *`maxValidators` reparameterization (PR #654).* `Topup2DistributionTx.allocate` and the parent statement now take a caller-supplied `maxValidators : Nat`; legacy callsites pass the module constant `maxValidatorsPerTopUp := 32` positionally so the CHECKED shape stays the same, but the theorem now admits any admissible `uint64` configuration of the pinned gateway. Sourced from the pinned `uint64 $.maxValidatorsPerTopUp` at `TopUpGateway.sol:42` (packed slot 0 of `TopupPackedStorage`). This partially addresses issue 24 above ("`maxValidatorsPerTopUp` is not a Verity guard"): the guard is now inside `allocate`, no longer only inside `well_formed_batch`.

(b) *D-SLASH-1 discharge (PR #660).* `evaluateTopUpLimit` and `sourceLimits{Independent,}` now take a caller-supplied per-validator `slashedOrExited : List Bool` that models the pinned early-return `if (exitEpoch != FAR_FUTURE_EPOCH || slashed) return 0;` at `TopUpGateway.sol:403-405`. The parent threads `slashedOrExited` through `allocate → sourceRun → sourceLimits`; a length mismatch against `effective` returns `none` from `sourceLimits` (both sides revert). Sourced upstream from the pinned SSZ validator proof (activation/slash/exit flags flow from beacon state).

**Open D-* divergences (Grok #417, mandate 2026-09-13).** The parent still uses the leftover-budget walk `sourceConsume` and gwei-scale arithmetic, so several divergences from the live gateway remain:

- **D-CONSUME-1** (`TopUpGateway.sol:226`): the pinned gateway computes `topUpLimits[i]` independently per validator via `_evaluateTopUpLimit(vw, pending[i]) * 1 gwei`, then hands the full `topUpLimits` array (already in wei) to `stakingRouter.topUp`; the router is what performs any budget allocation. The registered model still runs `sourceConsume` (a leftover-budget walk) on the gateway plane, coupling per-index outcomes to a shared budget order the gateway does not enforce. Related to issues 18 and 29 above.
- **D-UNITS-1** (`TopUpGateway.sol:226`, `* 1 gwei`): pinned `topUpLimits[i]` is in wei; model `evaluatedLimits[i]`, `budget`, `used`, `allocs`, `remaining` are all gwei-scale, and the `evaluatedLimits != topUpLimits` comparison at `sourceRunIndependent` therefore compares different units. Related to issues 21 and 27 above; a full unit propagation through the pipeline is required to close it.
- **D-TOTAL-1** (`TopUpGateway.sol:234-236`, `if (totalLimits > 0) _setLastTopUpData();`): the pinned gateway only writes `lastTopUpBlock`/`lastTopUpTimestamp` when at least one `topUpLimits[i]` is nonzero. The registered `allocate` has no `lastTopUp` field and its `View` has no `didSetLastTopUpData` observable, so a zero-limit batch is indistinguishable from a non-zero one. Directly restates issue 31 above.
- **D-AUTH/SORT/WC/PUBKEY/MAX prefix** (`TopUpGateway.sol:174-231`): `onlyRole(TOP_UP_ROLE)`, strictly-increasing `validatorIndex` (`InvalidValidatorIndicesSortOrder`), `withdrawalCredentialsType == 0x02`, `_verifyValidator` pubkey/proof correspondence, and `count > $.maxValidatorsPerTopUp` are all live gateway guards. Wave-3 (b) adds the count guard as a caller-supplied premise; the other four guards remain out of scope of the registered parent (`well_formed_batch` conjuncts are not threaded into `Topup2DistributionTx.allocate`, per issue 22 above).

**Grok #417 harness state.** Independent differential vectors for the D-* items above live on the `grok/lido-topup2-*` branches; a worktree checkout of that branch with `origin/main` merged is the required harness site (per the 2026-09-13 rule 9). Grok #407 (wei conversion, `allocateDeposits` policy) awaits review; the CLEAN verdict is a prerequisite to integration into `make test`.

**Ranked next work (updated 2026-09-13).** (1) Retire `sourceConsume` and re-shape the parent to model per-`topUpLimits[i]` independent evaluation, matching `TopUpGateway.sol:226` (D-CONSUME-1). (2) Thread `* 1 gwei` unit conversion through the parent so `topUpLimits`/`budget`/`used`/`allocs`/`remaining` all live in wei (D-UNITS-1). (3) Add a `didSetLastTopUpData : Bool` observable gated on `totalLimits > 0` (D-TOTAL-1). (4) Land the remaining D-AUTH/SORT/WC/PUBKEY prefix as caller-supplied premises. (5) Turn each closed D-* into a green vector on the Grok #417 harness before declaring the divergence closed (rule 9).
