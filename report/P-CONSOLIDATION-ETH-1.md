...
The registered abstract theorem is `eth_flow_parent_at_canonical`, a real $\forall (\mathrm{msgValue}, n, \mathrm{fee})$ whose `canonicalApprovedSet` fixes the fee destination to the pinned EIP-7251 source literal `0x0000BBdDc7CE488642fb579F8B00f3a590007251`. Equality of a deployed configured target with that model literal (`A-CANONICAL-REQUEST-ADDRESS`) is DISCHARGED via `Provenance/CanonicalRequestAddress.lean` + `scripts/verify_consolidation_request_immutable.py` (chantier 4, Thomas 2026-09-13). The registered Verity theorem is `verity_tx_success_and_revert_partition`, which conjoins `verity_tx_universal_success_shape` on the success arm with `UniversalRevertPartition` on all four modeled non-success arms; both sides match the abstract plane's quantifier strength under explicit non-revert and fuel premises. Chantier 2 (Thomas 2026-09-13, item a): the parent's per-request fee input has been wired to `PredeployStaticcallResult.abiDecodedFee` via PR #687 (registered-parent statement change naming the pinned EIP-7251 STATICCALL return). Former P-CONSOLIDATION-ETH-1a (vault→Lido/WQ returns) remains retired, and no executable-frame composition into P-CONSOLIDATION-1 is claimed (multi-session Verity model work).
...
Ranked next work: keep the universal success parent, the universal revert partition, and the premise kill-lines. Deployed-target provenance from artifacts was DISCHARGED in chantier 4 via `Provenance/CanonicalRequestAddress.lean` + `scripts/verify_consolidation_request_immutable.py`. The `verity_tx_success_and_revert_partition` fee input has been wired to `PredeployStaticcallResult` via PR #687. Remaining OPEN follow-ups: full executable-frame `Contract.run` composition into P-CONSOLIDATION-1 via `abi.encodeCall`; live-STATICCALL executable model on the pinned EIP-7251 predeploy to discharge the residual `hFeeNonzero : result.abiDecodedFee ≠ 0` premise.
