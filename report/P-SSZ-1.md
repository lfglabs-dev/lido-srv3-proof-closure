...
Ranked next work: keep the swapped-combine kill-line; YAML now splits opaque SHA-256 functional correctness from refinement to `Sha256Engine.sha256`; carry one-object coupling onto the TX input; keep Yul visibly OPEN next to every CHECKED presentation.
