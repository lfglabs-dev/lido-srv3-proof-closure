...
MODULES = [
    'audit.trio.consolidation.SettlementRequests',
    'LidoSRv3.Audit.Guarantees.PConsolidationEth1Requests',
...
for module, path in sorted(sources.items()):
    ...
    if '/.lake/packages/' in absolute:
        ...
        if relpath not in NEW:
            assert git(ROOT, 'show', 'f8ea5f9d3cffc165f5d84dd96444836c806827eb:' + relpath) == data, relpath
        local = True
