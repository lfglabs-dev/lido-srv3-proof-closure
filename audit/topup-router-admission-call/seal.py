...
sources=['LidoSRv3/Audit/Source/TopupRouterAdmissionCallGates.lean','LidoSRv3/Audit/Source/TopupRouterAdmissionCall.lean','LidoSRv3/Audit/Guarantees/PTopupRouterAdmissionCall.lean','LidoSRv3/Tests/TopupRouterAdmissionCallRegression.lean']
paths=[ROOT/p for p in sources]+[p for p in OUT.rglob('*') if p.is_file() and p.name!='receipt.json' and '__pycache__' not in p.parts]
# Solidity receipt is included; only the root receipt excludes itself.
paths+=[OUT/'solidity/receipt.json']
