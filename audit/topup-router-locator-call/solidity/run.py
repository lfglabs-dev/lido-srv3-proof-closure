...
NEW='audit/topup-router-locator-call/solidity/TopupRouterLocator.t.sol'
...
with tempfile.TemporaryDirectory(prefix='lido-locator-forge-') as temp:
 ...
 settings='[profile.default]\nsrc = "contracts"\ntest = "audit/topup-router-locator-call/solidity"\nlibs = []\noptimizer = true\noptimizer_runs = 200\nvia_ir = true\nevm_version = "cancun"\n'
 (d/'foundry.toml').write_text(settings)
 command=['forge','test','--root',str(d),'--use',str(solc),'--offline','--match-contract','TopupRouterLocatorTest','--match-test','test_locator_','-vv']
 ...
 artifacts={}
 for name in ['GatewayWitnessHarness','ExactRouterLocator','TopupRouterLocatorTest']:
 ...
 receipt=dict(scope='Fresh8 runtime diagnostics on actual inherited Gateway, arbitrary locator fallback and recorder router, independent SHA roots. Fixture profile viaIR optimizer200 Cancun; not production bytecode identity or a Lean proof.',
