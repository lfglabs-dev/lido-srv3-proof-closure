...
def diagnostics : IO Unit := do
  ...
  let e := {original with
    ...
    credentials := 999}
  let go := fun rootEnv amounts => TopupRouterLocatorCall.run LidoSRv3.Tests.TopupRouterLocatorCall.good
    (address 9) (word 128) (word 128)
    LidoSRv3.Tests.TopupPhysicalCredentialGetter.hash
    (TopupRootCallEffectsRegression.module amounts)
    TopupRouterContinuationMutants.callee rootEnv LidoSRv3.Tests.TopupRouterLocatorCall.supplied (address 8) (word 7)
    [word 1,word 2,word 3] [word 4,word 5,word 6] rows (word (2^256-1))
  let result := go e [word (10^18),word 0,word (2*10^18)]
  unless result.outcome == .ok () && result.locatorAttempts.length == 1 do
    throw (IO.userError s!"actual locator positive {repr result.outcome}")
  let some good := result.suffix | throw (IO.userError "missing actual suffix")
  ...
  let zeroResult := go e [word 0,word 0,word 0]
  let some zeros := zeroResult.suffix | throw (IO.userError "missing zero suffix")
  ...
  unless bad.outcome == .error (.phase (.phase (.batch (.gateway (.verifier (.proof .invalidProof)))))) do
  ...
  unless wrongRoot.outcome == .error (.phase (.phase (.batch (.gateway (.verifier (.proof .invalidProof)))))) do
  ...
  unless blocked.outcome == .error (.timing .arithmetic) && blocked.locatorAttempts.length == 0 && blocked.suffix.isNone do
    throw (IO.userError "distance subtraction must precede absent getter registration")
  let lateResult := go e [word (2*10^18),word (2*10^18),word 0]
  let some late := lateResult.suffix | throw (IO.userError "missing late suffix")
  ...
  unless overflow.outcome == .error (.timing .arithmetic) && overflow.locatorAttempts.length == 0 do
    throw (IO.userError "uint64 root age overflow")
  let redirected := TopupRouterLocatorCall.run (LidoSRv3.Tests.TopupRouterLocatorCall.getter (encode 32 4))
    (address 9) (word 128) (word 128) LidoSRv3.Tests.TopupPhysicalCredentialGetter.hash
    (TopupRootCallEffectsRegression.module [word 0,word 0,word 0]) TopupRouterContinuationMutants.callee e ctx (address 8) (word 7)
    [word 1,word 2,word 3] [word 4,word 5,word 6] rows (word (2^256-1))
  unless redirected.outcome == .error (.phase (.phase (.lookup (.bubbled TopupPhysicalCredentialGetter.unregistered)))) &&
      redirected.locatorAttempts.length == 1 && redirected.suffix.map (·.rootAttempts.length) == some 0 do
    throw (IO.userError "actual locator address not consumed by qualified physical registration")
  IO.println "PASS 8 fresh FFI diagnostics: actual-root positive physical suffix/history; actual-root zero allocations/history; altered WC; altered root; temporal guard before missing registration; late failure rollback; uint64 age overflow; changed locator selects absent physical router"
#eval diagnostics
