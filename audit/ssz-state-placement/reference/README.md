...
The recorded Node run used an isolated /tmp/lido-ssz-state-reference install with byte-identical check.mjs/schemas.json/package-lock.json and copied its deterministic outputs back here. Forge ran from the repository root with --root audit/ssz-state-placement/reference. receipt.json records actual commands, versions, source/dependency identities and hashes; validation.log and solidity-validation.log are actual successful outputs. No main/root dependency files were changed.
