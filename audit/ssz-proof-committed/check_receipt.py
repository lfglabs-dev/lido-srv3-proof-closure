...
for path,want in json.loads((here/'inherited-validation.json').read_text())['identity_checks'].items():
 checks+=1;p=root/path
 if not p.is_file() or hashlib.sha256(p.read_bytes()).hexdigest()!=want:errors.append('inherited:'+path)
