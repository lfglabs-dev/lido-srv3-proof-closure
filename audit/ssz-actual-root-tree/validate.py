...
MODULES = [
    'LidoSRv3.Audit.Source.SszActualLeafTree',
    'LidoSRv3.Audit.Guarantees.PSsz1ActualRootTree',
    'LidoSRv3.Tests.SszActualRootTreeRegression',
]
...
for module, path in sorted(sources.items()):
    ...
    if '/.lake/packages/' in absolute:
        ...
        if relpath not in NEW:
            assert git(ROOT, 'show', '14a96a3180ac9fb81f853db36af3b630c1d5891f:' + relpath) == data, relpath
        local = True
