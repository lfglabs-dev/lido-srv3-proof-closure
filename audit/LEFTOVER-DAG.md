...
| --- | --- | --- | --- | --- |
| G-DEPOSIT | `leftover-g-deposit` | `Audit/Provenance/Deposit.lean`, `Tests/PackGDepositProvenanceMutants.lean`, `audit/PACK-G-DEPOSIT-BRIEF.md` | Model pins equal production literals; a conserving config at 32e18 exists. `A-DEPOSIT-CONTRACT` DISCHARGED via `verify_beacon_deposit_immutable.py`; `A-DEPOSIT-32-ETHER` DISCHARGED via `verify_deposit_thirty_two_ether.py` (chantier 4). | Add decls to `PDeposit1.lean`. ALLOC→LinksSource. |
| G-TOPUP | `leftover-g-topup` | `Audit/Provenance/TopupBeacon.lean`, `Tests/PackGTopupProvenanceMutants.lean`, `audit/PACK-G-TOPUP-BRIEF.md` | `TopupTx.beaconAddress` equals the production beacon pin. `A-TOPUP-BEACON-ADDRESS` deployed-immutable identity is DISCHARGED via `scripts/verify_beacon_deposit_immutable.py` (fixture-anchored codehash for `StakingRouter_implementation`; see `audit/artifacts.lock.json`). | Add decls to `PTopup1.lean`. Top-up LinksSource from ALLOC. |
| G-ETH1 | `leftover-g-eth1` | `Audit/Provenance/ConsolidationRequest.lean`, `Tests/PackGEth1ProvenanceMutants.lean`, `audit/PACK-G-ETH1-BRIEF.md` | Unregistered child: ensemble `requestAddr` (5) maps to `canonicalRequestAddress` in a Spec-shaped observe rewrite. Registered Verity parent stays on ensemble 5. `A-CANONICAL-REQUEST-ADDRESS` deployed-target identity is DISCHARGED via `Provenance/CanonicalRequestAddress.lean` + `scripts/verify_consolidation_request_immutable.py` (chantier 4); the residual is the Verity-plane fold `MultiContract.requestAddr = 5` not folded to the discharged canonical literal. | Change registered ETH-1 parent. Compose with P-CONSOLIDATION-1. VaultHub. `native_decide` on parents. |
...
| C-GAP | `leftover-c-gap` | `Audit/Spec/ConsolidationBridgeGap.lean`, `Tests/PackCGapMutants.lean`, `audit/PACK-C-GAP-BRIEF.md` | Name the official-denotation gap: `Expr.call` reverts. `A-CONSOLIDATION-GATEWAY-NONZERO` is RETIRED from both consumer guarantees (PRs #699, #712) via `..._from_gateway` parent-statement swaps; the residual is the outer `hFeeNonzero : result.abiDecodedFee ≠ 0` premise pending a live-STATICCALL executable model. | Start the bus. Compose ETH-1 with CONSOLIDATION-1 via ABI encoder. Live-STATICCALL executable model to discharge `hFeeNonzero`. |
