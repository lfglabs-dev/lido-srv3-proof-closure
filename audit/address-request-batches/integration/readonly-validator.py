...
root=Path.cwd().resolve(); out=root/'audit/address-request-batches'; captured={}; old=Path.write_text
...
Path('/tmp/lido-address-batches-integration-identities.json').write_text(json.dumps({'source_commit':'274d8cc9aa543aa4aafda84937689676e0839cfe','sources':len(captured['source-inputs.json']['entries']),'pins':len(captured['source-inputs.json']['package_pins']),'artifact_hashes':len(r['hashes']),'scoped_axioms':len(captured['axioms.json']),'normal_olean_sha256':normal},indent=2)+'\n')
Path('/tmp/lido-address-batches-integration-scoped-axioms.json').write_text(json.dumps(captured['axioms.json'],indent=2)+'\n')
