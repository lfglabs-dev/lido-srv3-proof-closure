...
assembly = (OUT / 'solidity/WrappedRequestHarness.asm').read_text()
rows = []
...
expected = subprocess.check_output(['cast','sig','unwrap(uint256)'],text=True).strip()
wrapped = (ROOT/'LidoSRv3/Audit/Source/AddressWrappedRequestCalls.lean').read_text()
assert 'encode 4 '+expected in wrapped and expected in assembly
rows.append(dict(name='unwrapCalldata',signature='unwrap(uint256)',value=expected))
