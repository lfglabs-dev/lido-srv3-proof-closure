...
| `P-SSZ-1` | CHECKED | CHECKED | SSZ.verifyProof on production gindices; SHA-256 functional correctness; imported-to-deployed helper/wrapper Yul fragment binding; one-object composition across all four children (GIndex.concat and the digest/tx inputs remain independent of the deposit); 8-byte LE amount and ByteArray identity between children | IMPLEMENTATION_PENDING | `A-SHA256-FFI`, `A-MULTI-NODE-TRANSPORT`, `A-SOLC-TRUSTED`, `A-YUL-INTERFACE` |
