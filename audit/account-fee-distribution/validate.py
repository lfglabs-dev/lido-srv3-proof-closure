...
for module in ['LidoSRv3.Audit.Guarantees.PAccount1CheckedFeeSplit','Tests.Verity.ReportFeeCheckedSplitTest']:
...
(OUT/'solidity-identities.json').write_text(json.dumps(dict(pin=PIN,source_sha256=solidity,fresh_execution=False,scope='Pinned complete source identity for new typed distribution; fresh full-StETH/Accounting-fragment fixture validation is retained separately in solidity/validation.json.'),indent=2)+'\n')
...
computed=checker.environment_dependencies(names,'Tests.Verity.ReportFeeDistributionTest',None)
