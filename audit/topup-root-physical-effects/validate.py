...
MODULES = [
    'LidoSRv3.Audit.Source.TopupRootCallEffects',
    'LidoSRv3.Audit.Guarantees.PTopup1RootCalls',
    'LidoSRv3.Tests.TopupRootCallEffectsRegression',
]
...
for module, path in sorted(sources.items()):
    ...
    if '/.lake/packages/' in absolute:
        ...
        if relpath not in NEW:
            assert git(ROOT, 'show', '09a09ad875f8e1519586a04932efd4dbacdf713f:' + relpath) == data, relpath
        local = True
...
(OUT / 'source-inputs.json').write_text(json.dumps(dict(entries=entries, package_pins=pins,
    scope='Actual three-target setup import closures; inherited local sources match base; package sources match pinned git bodies.'), indent=2) + '\n')
