...
theorem deposit_execution_composes_suffix
    ...
    let result := Live.run (suffix external ctx execution.values) world
    DescribesSuffix external ctx execution.values world result.world result.outcome result.attempts := by
  have bounds := abi_success_composes_deposit_values layout storage oracle config
    requested before after moduleId limits obtainDepositData depositSize withdraw execution executed
  by_cases hzero : execution.values.actualKeys = 0
  · simp only [suffix, DescribesSuffix, hzero, if_pos]
    refine ⟨rfl, rfl, rfl⟩
  · simp only [suffix, DescribesSuffix, hzero, if_false]
    refine ⟨Nat.mod_eq_of_lt bounds.2.2.1, Nat.mod_eq_of_lt bounds.2.2.2, ?_⟩
    exact WithdrawalCalls.complete external ctx (amount execution.values)
      (seedDepositsCount execution.values) world
