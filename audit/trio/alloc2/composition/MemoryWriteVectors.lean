...
namespace LidoSRv3.Audit.Source.TrioAlloc2.MemoryWriteVectors
open MemoryWrite
private def w := TrioAlloc1.word
private def json (values : List Word) : String :=
  "[" ++ String.intercalate "," (values.map fun x => "\"" ++ toString x.val ++ "\"") ++ "]"
private def seed (ap cp : Nat) (buckets capacities : List Word) : TrioAlloc1.MemoryWords :=
  writeWords (writeWords (fun _ => w 0xcafe) ap (w buckets.length :: buckets))
...
private def emit (name : String) (ap cp : Nat) (bs cs : List Nat) (d1 d2 : Nat) : IO Unit := do
  ...
  | .ok (firstAmount, firstMemory) =>
    ...
    | .ok (secondAmount, secondMemory) =>
      ...
      IO.println ("ALLOC2_MEMORY_SEQUENCE {\"name\":\"" ++ name ++ "\",\"buckets\":" ++ json buckets ++
...
end LidoSRv3.Audit.Source.TrioAlloc2.MemoryWriteVectors
