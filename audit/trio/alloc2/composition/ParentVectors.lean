...
namespace LidoSRv3.Audit.Source.TrioAlloc2.ParentVectors
...
private def emit (name : String) (count share0 share1 unit amount : Nat)
  ...
  let (result, trace) := Parent.run layout (storage count share0 share1) (oracle s0 s1 reject1)
  ...
  IO.println ("ALLOC2_PARENT_VECTOR {\"name\":\"" ++ name ++ "\",\"actual\":\"" ++ hex bytes ++
...
end LidoSRv3.Audit.Source.TrioAlloc2.ParentVectors
