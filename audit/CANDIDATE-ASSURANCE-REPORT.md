...
- EXTERNAL-SUMMARY-INVARIANTS: SRLib.sol374-378 returns three uint256 values, consumed at517-522. ABI validity alone does not imply deposited>=max(summaryExited,accountingExited), nor bounds on deposited/depositable/totalStake. These are external module semantic obligations, not router-local write invariants. An unrestricted deployed-code proof is excluded; unconditional CheckedBounds is not asserted.
- INPUT/ARITHMETIC: cfg.maxEBType1 must be nonzero and input/allocation sums, available arithmetic and target products need the stated bounds. #652/#653 prove implications from explicit uint64/count assumptions; they do not derive those assumptions from this uint256-return interface.
- CALL-ORDER: bindLiveAll hoists all external summaries/stakes before sourceExecute. Solidity interleaves them with arithmetic in its first loop, so earlier arithmetic failure can prevent later calls. The current parent does not establish full source call-trace/error-order equivalence.
- PHYSICAL-STORAGE: AllocationTx uses model-local slot/map indices and observation arrays. Packed field decoding is covered; correspondence of those cells to physical ERC7201 storage and emitted runtime is not. Unlike DEPOSIT/TOPUP, this row is not relabeled a physical-world consumer.
- REGISTRY-REACHABILITY: SRLib._addModule checks duplicate addresses at200-205, but migration copies legacy addresses at101-113. A complete uniqueness invariant needs the initial/migrated registry and writer history. It is not a premise needed to claim unconditional allocation success here, nor derived by the current mapped-call theorem.
- SCOPE: getDepositAllocations caller composition, P-ALLOC-2 and ALLOC-to-DEPOSIT execution remain separate. General Yul/EVM/deployment closure is excluded.
