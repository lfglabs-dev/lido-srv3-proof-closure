...
**Proof shape / exact domain statement.** The executable parent consumes the actual typed TOPUP admission/root/module/per-key/history chain with concrete physical Keccak: gateway caller role/resume, locator and router caller admission calls, checked witness-derived wei limits, actual module-return allocations and physical withdrawal/beacon/history effects, or complete root rollback. Allocation-view calls and outer gateway-to-router ABI transport remain explicit boundaries. Historical abstract and legacy journal theorems remain available; free Bool naming wrappers do not establish the new parent.
...
- CALLEE/DEPLOYMENT: Lido withdrawal accounting executes, but its getter/receiver External still needs correspondence to the named actual callees. Arbitrary module/callback effects are consumed rather than assumed to preserve a pre-call ledger. The physical beacon helper is a source-shaped callee model, not a general deployed code/LOG ABI/gas proof.
...
- Arbitrary successful callbacks may spend their credited ETH or modify claim storage. Final recipient net credit and final claimed-bit preservation are not asserted; the precise returned world is retained instead.
