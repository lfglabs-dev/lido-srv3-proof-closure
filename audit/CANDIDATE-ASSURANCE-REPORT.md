...
**Proof shape / exact domain statement.** The abstract real_validator_correspondence theorem restates the chosen model’s validator-leaf, generalized-index, branch-fold and canonical-address definitions. It does not prove Solidity correspondence. The registered Verity result actual_compiled_cl_entry_complete_declared_branch establishes the successful compiled harness execution’s decoded inputs, BEACON_ROOTS call, validator layout and complete declared Merkle branch. SHA-256 correctness, authentic beacon history and deployed-bytecode correspondence remain explicit boundaries.
...
- A-SHA256-FFI: DepositDataRootCorrespondence.sha256 and the EvmYul SHA-256 FFI are not proved equivalent to the deployed precompile. Structural oracle-parametric proofs do not establish that correspondence.
