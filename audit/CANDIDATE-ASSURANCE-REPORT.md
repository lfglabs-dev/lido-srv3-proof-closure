...
- Other writer bodies (NAMED GAP 2026-09-21, not composable from existing theorems: the only LiveRenaming theorems in the tree are the claim batch's runClaimWithdrawalsTo_rename / actual_claim_batch_rename; no transferFrom, requestWithdrawals or unwrap live-body renaming theorem exists to compose, so the site's remaining Not-proven bullet stays; closing it needs new live-body proofs, and no named assumption can honestly discharge it because the missing content is the theorem itself): the four-input projection still carries environment booleans. Full requestWithdrawals/unwrap/token-transfer and ERC-721 approval-branch state/callee equivariance is not established by the claim-batch limb.
...
**Next gate.** 2026-09-21: the strengthened registered parent (PAddress1) and the cleanup composition modules (PAddress1LiveRenaming, PAddress1NoReentry, the A-NO-REENTRY / A-SUPPORTED-MODULES compositions and the gateway witness admission) build at exact head a06dc494 on dgx-spark (jobs 9e12e8a0-4b33-4113-b1bf-890827a9efea and c328f9a3-1254-494a-bde3-d8275f5dd2ac, exit 0). 2026-09-19: live sender/owner renaming of the claim batch registered (PAddress1LiveRenaming), with the abstract LiveRenaming and the callee-equivariance hypothesis disclosed. Remaining named gap: live equivariance of the full transferFrom, requestWithdrawals and unwrap bodies is not composable from existing theorems (see fidelity.missing). Then independently audit the actual physical claim/CALL/rollback composition. Global live-world renaming and the named deployed/external-interface obligations remain open; do not promote projection or legacy-journal results to those claims.
...
- SHA-256 functional correctness of the opaque model/precompile symbol (PERMANENT ASSUMPTION A-SHA256-FFI, relabelled 2026-09-19 from an open obligation to the card's Assumed line; no refinement is claimed)
...
- the state-root anchor (lhs), the concat operand's pow byte (rhsPow), and the tx child's fork version / claimed root remain independently supplied, non-deposit chain-level values (composition half CLOSED 2026-09-19: the verifier's typed model is consumed inside the whole consolidation gateway run by GatewayWitnessAdmission / WitnessProof.validateAll against the EIP-4788 root read in that run; residual: the deposit chain's anchor, pow byte and fork/claimed-root values stay supplied inputs of the SSZ child)
...
- entry consensus hash is an opaque word equality, not keccak256 of ABI-encoded calldata, and extra-data checks are not modeled (contract-version, consensus-version, ref-slot and processing-deadline checks EXECUTED 2026-09-21 by POracleSupply1EntryGuards: PAccount1SubmitReportGuards.submitReportDataGuarded runs the pinned AccountingOracle.submitReportData ladder on the same SubmitReportEntry.SubmitReportData this row's entry consumes, and supply_entry_under_executed_guards / supply_entry_source_domain_under_executed_guards derive this row's registered entry conjuncts for every guarded run that reaches a body, so senderAllowed and consensusHashMatches are conclusions of the executed ladder rather than supplied premises. The oracle words the ladder reads stay inputs. NOT retired, still open on this row: the data hash is compared as an opaque word (consensusHashMatches d is d.dataHash == d.consensusHash), no keccak256 preimage of the ABI-encoded report calldata is modeled, and extra-data processing remains outside the modeled body)
...
**Classification.** **IMPLEMENTATION_PENDING** — Keep P-ACCOUNT-1 order-only; do not treat the Pack E argument-as-mint child as this parent; the entry ladder's version/ref-slot/consensus-version/deadline checks are executed by POracleSupply1EntryGuards, and the remaining opaque consensus-hash equality, extra-data processing and E27 quantization stay disclosed.

**Next gate.** 2026-09-21: the pinned entry ladder's contract-version, consensus-version, ref-slot and processing-deadline checks are executed, not assumed (POracleSupply1EntryGuards composes PAccount1SubmitReportGuards with this row's entry parent on the shared SubmitReportData); the registered parent is unchanged. Residual disclosures: the consensus hash stays an opaque word equality with no keccak256 preimage, extra-data processing stays outside the modeled body, and the E27 quantization stays disclosed. Keep P-ACCOUNT-1 order-only; do not treat Pack E argument-as-mint as this parent.
