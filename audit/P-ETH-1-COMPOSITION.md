...
Parent `P-ETH-1` is `CHECKED` on both planes. There are no sibling guarantee
rows under this parent.

- **Former P-ETH-1b** (fee → configured consolidation-request target) is
  ...
  sister claim.
- **Former P-ETH-1a** (vault → Lido / WithdrawalQueue returns) is **retired**
...
`PEth1RequestTx.consolidation_fee_target_success` is parent evidence, not a
child registry theorem. Refund/vault-to-Lido numerals in `PEth1RefundTx`
...
reintroduce a P-ETH-1a child for vault→Lido/WQ returns without opening a
separate guarantee.
