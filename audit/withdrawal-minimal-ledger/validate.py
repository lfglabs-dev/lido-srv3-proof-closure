...
MODULES = ['audit.trio.deposit.WithdrawalLedgerMinimal', 'audit.trio.deposit.PhysicalMetadataLedger', 'LidoSRv3.Audit.Source.TopupPipelineMinimalLedger', 'LidoSRv3.Audit.Guarantees.PTopup1MinimalLedger', 'LidoSRv3.Audit.Guarantees.PDeposit1PhysicalLedger', 'LidoSRv3.Tests.WithdrawalMinimalLedgerRegression']
...
for module, path in sorted(sources.items()):
    ...
    if '/.lake/packages/' in absolute:
        ...
        if relpath not in NEW:
            assert git(ROOT, 'show', 'b45cf453834a628439c9899feba760a77d3b984d:' + relpath) == data, relpath
        local = True
