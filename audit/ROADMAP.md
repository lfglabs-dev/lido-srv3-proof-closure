...
- `P-DEPOSIT-1` — **IMPLEMENTATION_PENDING**: Keep LinksSource explicit. Discharge OPEN A-DEPOSIT-CONTRACT and A-DEPOSIT-32-ETHER from deployment artifacts. The two-batch limitation is now explicit in the registered parent (conjunct (d)) and checked by abstract_parent_covers_deployments_the_verity_plane_omits; closing it requires reshaping TX to one-module/n-frames with an inductive frame correspondence.
