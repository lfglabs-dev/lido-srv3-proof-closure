...
- `P-DEPOSIT-1` — **IMPLEMENTATION_PENDING**: Keep LinksSource explicit. Discharge OPEN A-DEPOSIT-CONTRACT and A-DEPOSIT-32-ETHER from deployment artifacts. The two-batch limitation is closed by DepositNFrameTx / verity_tx_composes_nframe_deposit; remaining gaps are the caller-hypothesis bridge, the two named deployment constants, the vacuous internal revert conjunct, and the unbounded-Nat versus word gap. Do not hide those behind the n-frame title.
