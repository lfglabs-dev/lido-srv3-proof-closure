<!-- GENERATED by scripts/audit_metadata.py; edit structured metadata, not this view. Lean is theorem authority. -->

# ROADMAP

## P0 — keep published claims honest

- Do not promote a parent guarantee without a composed theorem on the claimed plane.
- `make test` and `make prove` remain the local gates; metadata never closes evidence.

## P1 — first complete property: `P-RESERVE-RELATIONAL`

Fix report, queue, and buffer. Two states that differ only in `depositsReserve` must yield the same prefinalized/finalized ranges and the same locked ETH. Checked `P-RESERVE-1` spending is a child, not this fact.

Closed: a spec, an independently defined pinned-source interpreter, an executable Verity transaction that computes the five observables from storage and memory, the composition theorem, a rejected reserve→range mutant, report/queue/buffer mutants, and rollback after a mid-write. The parent is registered as a supplemental row now that composition exists.

## P2 — allocation and value conservation

Order: P-ALLOC-1/2, then deposit/top-up allocation, WC01/WC02 eligibility, Lido debit, Beacon credit, module delta, rollback. The P-RESERVE-RELATIONAL gate on other parent-closure lanes is now satisfied.

## P3 — remaining parents

Resume Accounting, Address, Topup2, Deposit, Topup1, ETH, Consolidation, and SSZ only with the composition patterns from P1/P2.

## Current guarantee registry

## Deferred P1-labelled registry rows


## Deferred P2-labelled registry rows


## Deferred P3-labelled registry rows


## Checked baseline to preserve

- `P-ALLOC-1` — **IMPLEMENTATION_PENDING**: Keep the checked theorems; close listed fidelity gaps without widening the claim.
- `P-ALLOC-2` — **IMPLEMENTATION_PENDING**: Keep the checked theorems; close listed fidelity gaps without widening the claim.
- `P-DEPOSIT-1` — **IMPLEMENTATION_PENDING**: Preserve the commit-branch-explicit abstract parent, its source-plane dropped-assert kill-line, and the composed Verity transaction evidence; the open work is the LinksSource/ALLOC composition, beacon-address provenance, and the two-batch unrolling.
- `P-TOPUP-1` — **IMPLEMENTATION_PENDING**: Preserve the composed P-TOPUP-1 Contract.run correspondence over journal and allocation observables, its intermediate-effect rollback, its executable mutants, and the four surgical abstract-plane kill-lines with their characterization theorems; close the remaining fidelity gaps (residual exact-reading guards, LinksSource, beacon-address provenance) without widening the claim.
- `P-ACCOUNT-1` — **IMPLEMENTATION_PENDING**: Keep the checked theorems and the mint-after-read kill-line on the registered parent; close listed fidelity gaps (starting with the slot-pinned reordering residual) without widening the claim.
- `P-RESERVE-1` — **IMPLEMENTATION_PENDING**: Keep the checked theorems; close listed fidelity gaps without widening the claim. Do not drop the freshQueueCache hypothesis from the parent theorem.
- `P-ETH-1` — **IMPLEMENTATION_PENDING**: Compose into P-CONSOLIDATION-1 only after FunctionSpec becomes ConsolidationGateway.addConsolidationRequests.
- `P-ADDRESS-1` — **IMPLEMENTATION_PENDING**: Keep the checked unconditional parent; close the remaining singleton-actor modeling gap without widening the claim.
- `P-TOPUP-2` — **IMPLEMENTATION_PENDING**: Keep the checked theorems; close listed fidelity gaps without widening the claim.
- `P-CONSOLIDATION-1` — **IMPLEMENTATION_PENDING**: Keep the checked theorems; close listed fidelity gaps without widening the claim. Do not compose with P-ETH-1.
- `P-SSZ-1` — **IMPLEMENTATION_PENDING**: Keep the checked theorems; close listed fidelity gaps without widening the claim.
