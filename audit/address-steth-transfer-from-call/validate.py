...
BASE='f1d91e3a8ad9829e92ab2e8ff443e391449a36bc'
MODULE='LidoSRv3.Tests.AddressStETHTransferFromCalls'
NEW=['LidoSRv3/Audit/Source/AddressStETHTransferFromCalls.lean','LidoSRv3/Audit/Guarantees/PAddress1StETHTransferFromCalls.lean','LidoSRv3/Tests/AddressStETHTransferFromCalls.lean']
READONLY='--readonly' in sys.argv
def emit(path,text):
 if READONLY:assert path.read_text()==text,path
...
for name in re.findall(r'^#print axioms (\S+)',(ROOT/NEW[-1]).read_text(),re.M):
 ...
 elif name.startswith('public_'):name='LidoSRv3.Tests.AddressStETHTransferFromCalls.'+name
 names.append(name)
...
emit(OUT/'axioms.json',json.dumps({n:sorted(a) for n,a in computed.items()},indent=2)+'\n')
...
emit(OUT/'runtime-inputs.json',json.dumps(dict(scope='Fresh diagnostic dylib only; not kernel theorem evidence',sha256=runtime,repositories={d:git(pkg/d,'rev-parse','HEAD').decode().strip() for d in ['sha2','keccak256']}),indent=2)+'\n')
...
for p in NEW:
 name=p[:-5]
 for rel in [p,'.lake/build/lib/lean/'+name+'.olean','.lake/build/lib/lean/'+name+'.ilean','.lake/build/lib/lean/'+name+'.trace','.lake/build/ir/'+name+'.setup.json']:
  normal.append(dict(path=rel,sha256=sha(ROOT/rel)))
for p in ['lakefile.lean','lake-manifest.json','lean-toolchain']:
 assert (ROOT/p).read_bytes()==git(ROOT,'show',BASE+':'+p)
 normal.append(dict(path=p,sha256=sha(ROOT/p)))
emit(OUT/'normal-build-identities.json',json.dumps(normal,indent=2)+'\n')
print('PASS 18 exact normal source/olean/ilean/trace/setup/config identities')
