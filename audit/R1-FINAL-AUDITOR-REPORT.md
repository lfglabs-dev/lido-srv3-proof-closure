...
- general-rule residual (Thomas 2026-09-12): the P-TOPUP-1 `PinnedSRTopupCallShape` scaffold (see covered) names each pinned SR read but does NOT yet execute them against live SR storage. A real derivation requires: (a) a live `SRStorage.getModuleState` source model to derive `moduleExists` and `moduleActive` from a keyed storage read; (b) an Aragon-ACL role-check chain to derive `callerIsTopUpGateway` from `_checkAppAuth(_getTopUpGateway())`; (c) a `WithdrawalCredentials.isType2` source model. None are tree-resident yet. Until these live source models are added, the three `SourceTopupInput` booleans remain free even under the scaffold — the scaffold's value is naming the composition entry point, not eliminating the free-boolean-in-parent condition.
...
- general-rule residual (Thomas 2026-09-12): the P-CONSOLIDATION-ETH-1 `PinnedFeeStaticcallShape` scaffold (see covered) names the pinned STATICCALL entry point but does NOT yet execute it against the live EIP-7251 predeploy. A real derivation requires a live-STATICCALL executable model on the pinned CONSOLIDATION_REQUEST address, an ABI decoder for the 32-byte fee return word, and an EIP-7251-schedule model to describe the predeploy's per-block fee output. None are tree-resident yet. Until they are added, the `feePerRequest` remains a free Nat even under the scaffold — the scaffold's value is naming the composition entry point.
...
- general-rule residual (Thomas 2026-09-12): the P-ADDRESS-1 `PinnedBridgeCallShape` scaffold (see covered) names the pinned Bridge CALL entry point but does NOT yet execute the per-writer Bridge-to-source glue lemmas. A real derivation of `Input.externalCallSucceeds` from the pinned bridge callee requires proving, for each of the four writers (requestWithdrawals, unwrap, claimWithdrawalsTo, transferFrom), that the corresponding `externalCallBindTo` frame's callee-success bit equals the source model's `externalCallSucceeds`. That per-writer glue is not tree-resident. Until it is added, the `externalCallSucceeds` boolean remains free even under the scaffold — the scaffold's value is naming the composition entry point, not eliminating the free-boolean-in-parent condition.
