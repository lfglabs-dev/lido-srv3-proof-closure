...
- chantier 5 CL proof guard (mandate 2026-09-12): the consensus-layer validator-status proof required by the gateway before forwarding source/target pubkey pairs to the vault is not yet modeled; a batch with unverifiable/invalid CL proof must revert before the modeled arms. Composition would extend `PhysicalEntrySettlement` with a per-group `_validatePubKeyWCProof` (ConsolidationGateway.sol:206-208) gate on the `withdrawalCredentials` returned by `_getWithdrawalVaultData` (line 203). Remains OPEN.
