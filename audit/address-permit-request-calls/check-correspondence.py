...
BASE='274d8cc9aa543aa4aafda84937689676e0839cfe';PIN='17005714f151e5502c559932319a3f2f74ac2436'
...
for folder,version,prefix,coreversion in [(OLD/'solidity/token/src','3.4.0','@openzeppelin/contracts/','0.6.12'),(S/'src','4.4.1','@openzeppelin/contracts-v4.4/','0.8.9')]:
 ...
 with tarfile.open(archive) as tar:
  for p in sorted(folder.rglob('*.sol')):
   ...
   if rel=='BatchHarness.sol':
    assert p.read_bytes()==old(R/'audit/address-request-batches/solidity/src/BatchHarness.sol').read_bytes();continue
...
selectors={sig:subprocess.check_output(['cast','sig',sig],text=True).strip() for sig in ['permit(address,address,uint256,uint256,uint8,bytes32,bytes32)','requestWithdrawalsWithPermit(uint256[],address,(uint256,uint256,uint8,bytes32,bytes32))','requestWithdrawalsWstETHWithPermit(uint256[],address,(uint256,uint256,uint8,bytes32,bytes32))']}
...
assert selectors['permit(address,address,uint256,uint256,uint8,bytes32,bytes32)']=='0xd505accf'
assert '0xd505accf' in (R/'LidoSRv3/Audit/Source/AddressPermitRequestCalls.lean').read_text()
prior=old(R/'audit/address-request-batches/compiler-artifact.json');current=json.loads((O/'compiler-artifact.json').read_text());priorData=json.loads(prior.read_text())
for key in ['bytecode','deployedBytecode']:assert current[key]['object']==priorData[key]['object']
assert current['metadata']==priorData['metadata']
assert (O/'BatchHarness.asm').read_bytes()==old(R/'audit/address-request-batches/BatchHarness.asm').read_bytes()
assert (S/'test/Batches.t.sol').read_bytes()==old(R/'audit/address-request-batches/solidity/test/Batches.t.sol').read_bytes()
result=dict(base=BASE,core_pin=PIN,resume_slot=slot,selectors=selectors,packages=packages,reused_inputs=reused,pinned_sources=sources,compiler_artifacts=artifacts,assembly_sha256=sha(O/'BatchHarness.asm'),scope='Full legacy assembly and exact source bodies/metadata. Permit transport/body boundary is external; existing typed-batch allocation/ABI/gas boundaries retained. Bytecode objects/metadata/assembly match prior batch; sourceMaps differ in fresh fixture artifact; no whole-artifact equality claimed.')
