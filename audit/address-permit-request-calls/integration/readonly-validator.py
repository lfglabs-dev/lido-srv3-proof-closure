...
root=Path.cwd().resolve(); out=root/'audit/address-permit-request-calls'; captured={}; old=Path.write_text
...
Path('/tmp/lido-address-permit-integration-identities.json').write_text(json.dumps({'source_commit':'6e87eb6e7d6465936d3fab76c3fc7fab5ec51194','sources':len(captured['source-inputs.json']['entries']),'pins':len(captured['source-inputs.json']['package_pins']),'artifact_hashes':len(r['hashes']),'scoped_axioms':len(captured['axioms.json']),'normal_olean_sha256':normal},indent=2)+'\n')
Path('/tmp/lido-address-permit-integration-scoped-axioms.json').write_text(json.dumps(captured['axioms.json'],indent=2)+'\n')
