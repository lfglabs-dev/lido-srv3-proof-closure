...
root=Path.cwd().resolve(); out=root/'audit/address-steth-conversion-call'; captured={}; old=Path.write_text
...
for rel in ['lakefile.lean','lake-manifest.json','lean-toolchain']:
 import subprocess
 assert (root/rel).read_bytes()==subprocess.check_output(['git','show','107e57e53ad4b1bd459a062d887bbcac85be6e41:'+rel]),rel
...
Path('/tmp/lido-address-conversion-integration-identities.json').write_text(json.dumps({'source_commit':'107e57e53ad4b1bd459a062d887bbcac85be6e41','sources':len(captured['source-inputs.json']['entries']),'pins':len(captured['source-inputs.json']['package_pins']),'artifact_hashes':len(r['files']),'scoped_axioms':len(captured['axioms.json']),'normal_olean_sha256':normal},indent=2)+'\n')
Path('/tmp/lido-address-conversion-integration-scoped-axioms.json').write_text(json.dumps(captured['axioms.json'],indent=2)+'\n')
