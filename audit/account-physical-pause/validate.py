...
MODULES={
 'ReportFeePhysicalPause':'audit/trio/account-address/ReportFeePhysicalPause.lean',
 'LidoSRv3.Audit.Guarantees.PAccount1PhysicalPause':'LidoSRv3/Audit/Guarantees/PAccount1PhysicalPause.lean',
 'LidoSRv3.Tests.AccountPhysicalPause':'LidoSRv3/Tests/AccountPhysicalPause.lean'}
...
for module in ['Tests.Verity.ReportFeeTreasuryCallTest','LidoSRv3.Audit.Guarantees.PAccount1TreasuryCall']:
...
(OUT/'source-inputs.json').write_text(json.dumps(dict(entries=entries,package_pins=pins,scope='Actual two prerequisite setup import closures plus three sequentially compiled new modules; every old local source body unchanged at base; cached setup absolute paths checked byte-identical against current source and actual local olean.'),indent=2)+'\n')
assert git(CORE,'rev-parse','HEAD').decode().strip()==PIN
corefiles=['contracts/0.8.9/Accounting.sol','contracts/0.8.25/sr/StakingRouter.sol','contracts/0.8.25/sr/SRLib.sol','contracts/0.4.24/Lido.sol','contracts/0.4.24/StETH.sol','contracts/0.4.24/utils/Pausable.sol']
solidity={}
for path in corefiles:
    data=(CORE/path).read_bytes();assert git(CORE,'show',PIN+':'+path)==data
    solidity[path]=sha(data)
(OUT/'solidity-identities.json').write_text(json.dumps(dict(pin=PIN,source_sha256=solidity,fresh_execution=False,scope='Pinned complete core source identities for physical pause; reused full-Lido37 compiler and fresh caller fixture checks are recorded in compiler-identities.json.'),indent=2)+'\n')
...
computed=checker.environment_dependencies(names,'LidoSRv3.Tests.AccountPhysicalPause',None)
