...
A test asks, ``does this example work?'' An executable model check asks, ``does
this rule survive the modeled transition and fixture or bounded enumeration that
the artifact names?'' That is why the report spends so much effort naming the
model, bounds, and assumptions: they define exactly what the local command
checks.
\end{notebox}
...
\begin{quote}
For the focused SRv3 economic state machine, the local model tests and bounded
proof harness preserve deposit conservation, reserve separation, module-balance
conservation, report-before-reward consistency, reward/fee bounds, and
flow-specific status gating over the named fixtures and enumerated domains.
\end{quote}
...
states what must not go wrong in the focused model. Third, the local harness
checks the Verity-style model against the explicit assumptions.
...
\path{verity/}, and \path{tests/verity/}.
