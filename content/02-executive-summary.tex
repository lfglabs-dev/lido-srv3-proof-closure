...
machine can check them for every modeled case, not only for examples? This
specimen shows the intended final-report answer once the companion proof
repository exists: the answer would be yes for the six selected P0
(highest-priority) properties, under the named assumptions in
\cref{sec:trust-boundary}.
\end{notebox}

\noindent In a final report, ``closed'' has a specific meaning: the theorem compiles
without admitted proof obligations, its Solidity-facing model target is listed,
all premises not discharged inside the model are registered by assumption ID, and
the reproducibility command rechecks the theorem namespace from a clean checkout. Because this
repository is a specimen, that sentence is the evidence threshold for a real
delivery rather than a claim that the proof repository is present here.
...
A test asks, ``does this example work?'' A theorem asks, ``does this rule survive
every modeled case that satisfies the assumptions?'' That is why the report
spends so much effort naming the model and assumptions: they define the world in
which the machine-checked statement is meant to be universal.
\end{notebox}

\smallskip
\noindent The final claim this specimen is designed to support is:

\begin{quote}
For all modeled finite module arrays and accepted report arrays satisfying the
listed well-formedness assumptions, the selected PR \#1811 SRv3 economic state
machine preserves deposit conservation, reserve separation, module-balance
conservation, report-before-reward consistency, reward/fee bounds, and
flow-specific status gating.
\end{quote}

\noindent This would not be a claim that every deployed-system behavior is proved. The
...
Read each row below as a three-step promise for the final package. First, the
Solidity-facing model names the transition being studied. Second, the property
states what must never go wrong for any modeled input. Third, Lean should check
the resulting theorem against the Verity-generated state model and the explicit
assumptions. The value of the exercise is not that one test passed; it is that
the statement leaves no remaining case split for a reviewer to hold in their
head.
...
\noindent All six rows below are specimen targets: they show the intended final
closed state once the corresponding theorem artifact and trust-boundary row are
present in the companion proof repository.
...
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.12\linewidth}>{\raggedright\arraybackslash}p{0.24\linewidth}>{\raggedright\arraybackslash}X@{}}
  \toprule
  \thead{ID} & \thead{PR \#1811 surface} & \thead{Intended final property} \\
  \midrule
...
This specimen is written for two readers at once. A protocol reviewer can follow
the ledgers and the trust boundary without reading Lean. A formal-methods
reviewer can check whether the theorem IDs, assumptions, and rebuild gates would
support the advertised claim in a real delivery package.
