...
for (const [entry, compilerName, evmVersion] of [
  ['KernelHarness.sol','solc-0424','constantinople'],
]) {
...
fs.writeFileSync(path.join(root,'audit/trio/reserve1/receipts/kernel-compilation.json'),
  JSON.stringify({pin,node:process.version,receipts,sourceHashes},null,2)+'\n');
console.log('Compiled pinned Aragon Kernel.');
