...
for (const [entry, compilerName, evmVersion] of [
  ['ACLHarness.sol','solc-0424','constantinople'],
]) {
...
fs.writeFileSync(path.join(root,'audit/trio/reserve1/receipts/acl-compilation.json'),
  JSON.stringify({pin,node:process.version,receipts,sourceHashes},null,2)+'\n');
console.log('Compiled pinned Aragon ACL.');
