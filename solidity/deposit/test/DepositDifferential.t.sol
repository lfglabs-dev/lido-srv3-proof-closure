...
contract DepositDifferentialTest {
    ...
    function modelJson(
        ...
        // Conserving deployment: `maxEBType1 == DEPOSIT_SIZE` (CLI default).
        return string.concat(
            "{",
    ...
    /// Skewed-deployment variant: emits an explicit `maxEBType1` (post D-SKEW-1).
    function modelJsonSkewed(
        bool authorized,
        bool moduleActive,
        bool allocationValid,
        bool lidoCallOk,
        ...
        return string.concat(
            ...
            ",\"lidoCallOk\":", jsonBool(lidoCallOk),
            ",\"depositSize\":", vm.toString(DEPOSIT_SIZE),
            ",\"maxEBType1\":", vm.toString(maxEBType1),
            ",\"lido\":", vm.toString(uint256(uint160(address(lido)))),
            ",\"module\":", vm.toString(uint256(uint160(address(module)))),
            ",\"beacon\":", vm.toString(uint256(uint160(address(beacon)))),
            ",\"batches\":", batches,
    ...
    function familyModel(string memory reason) internal pure returns (string memory) {
        // Post D-REVERT-1 the model reports pinned custom-error / Panic names.
        if (
            keccak256(bytes(reason)) == keccak256("NOT_AUTHORIZED")
                || keccak256(bytes(reason)) == keccak256("NotAuthorized()")
        ) return "auth";
        if (
            keccak256(bytes(reason)) == keccak256("MODULE_NOT_ACTIVE")
                || keccak256(bytes(reason)) == keccak256("StakingModuleNotActive()")
        ) return "inactive";
        if (
            keccak256(bytes(reason)) == keccak256("INVALID_ALLOCATION")
                ...
                || keccak256(bytes(reason)) == keccak256("ZeroDeposits()")
                || keccak256(bytes(reason)) == keccak256("Panic(0x11)")
        ) return "allocation";
        if (
            keccak256(bytes(reason)) == keccak256("ASSERT_BALANCE_UNCHANGED")
                || keccak256(bytes(reason)) == keccak256("Panic(0x01)")
        ) return "assert";
    ...
    function testUnauthorized() public {
        ...
        // D-REVERT-1 closed: the model now emits the pinned custom-error name.
        require(keccak256(bytes(model.revertText)) == keccak256("NotAuthorized()"));
        require(sol.selector == bytes4(keccak256("NotAuthorized()")), "D-REVERT-1 closed");
    }
    ...
    function testZeroDeposits() public {
        ...
        // D-REVERT-1 closed: model emits the pinned custom-error name.
        require(keccak256(bytes(model.revertText)) == keccak256("ZeroDeposits()"));
    }
    ...
    function testWrongPubkeyLength() public {
        ...
        // D-REVERT-1 closed: allocationValid=false path emits ZeroDeposits() —
        // the pinned StakingRouter also short-circuits before reaching the
        // per-key `WrongPubkeyLength()` inspection because `allocation.value`
        // is zero. The `allocation` family already agrees selector-wise.
        require(keccak256(bytes(model.revertText)) == keccak256("ZeroDeposits()"));
    }
    ...
    function testWordOverflowIsModelOnly() public {
        ...
        // Panic(0x11) on that Nat input via the wrapping fold. Documented
        // domain gap (post D-REVERT-1: the pinned Panic name is emitted).
        ...
        require(keccak256(bytes(model.revertText)) == keccak256("Panic(0x11)"));
    }

    function testDuplicateModuleIdsRejectedByModel() public {
        // D-NFRAME-1 closed: `execute` now enforces one moduleId per call via
        // an in-body `Nodup` guard on `batches.map moduleId`. The pinned
        // `StakingRouter.deposit` admits only one module per call, so the
        // Grok vector now agrees the model rejects duplicates in-body.
        ...
        require(!model.ok, "model execute rejects duplicate module ids at the in-body Nodup guard");
        require(keccak256(bytes(model.revertText)) == keccak256("DuplicateModuleId"));
    }
    ...
    function testSkewedMaxEbHitsConservationAssert() public {
        ...
        // D-SKEW-1 closed: the model now carries an immutable `maxEBType1`
        // separate from `depositSize` (mirroring the pinned constructor
        // immutable). The skewed vector reaches the line-996 assert and
        // reports `Panic(0x01)`, matching the pinned selector family.
        Obs memory model = runModel(
            modelJsonSkewed(true, true, true, true, string.concat("[", jsonBatch(1, 1, DEPOSIT_SIZE), "]"), 320 ether, 0, 64 ether)
        );
        require(!model.ok, "model conservation assert fires on skewed maxEB");
        require(keccak256(bytes(model.revertText)) == keccak256("Panic(0x01)"));
        lido.setRouter(address(router));
    }
